...
class BeltITTests {
    ...
    void givenListOfBelts_whenGetAllBelts_thenReturnBeltsList() throws Exception{
        ...
        listOfBelts.add(new Belt(1L,"MacBook Pro", "", Category.PROGRAMMING));
        listOfBelts.add(new Belt(2L,"iPhone", "", Category.PROGRAMMING));
    ...
    void givenListOfBelts_whenGetBeltsByCategory_thenReturnBeltsForCategory() throws Exception{
        // given - precondition or setup
        List<Belt> listOfBelts = new ArrayList<>();
        listOfBelts.add(new Belt(null,"One", "one", Category.SOFT_SKILLS));
        listOfBelts.add(new Belt(null,"Two", "two", Category.SOFT_SKILLS));
        beltRepository.saveAll(listOfBelts);
        // when -  action or the behaviour that we are going test
        ResultActions response = mockMvc.perform(get("/api/v1/belts")
    ...
    void givenListOfBelts_whenGetBeltsByInvalidCategory_thenReturnAllBelts() throws Exception{
        // given - precondition or setup
        List<Belt> listOfBelts = new ArrayList<>();
        listOfBelts.add(new Belt(null,"One", "one", Category.PROGRAMMING));
        listOfBelts.add(new Belt(null,"Two", "two", Category.PROGRAMMING));
        beltRepository.saveAll(listOfBelts);
        // when -  action or the behaviour that we are going test
        ResultActions response = mockMvc.perform(get("/api/v1/belts")
                .param("category", "fake"));
    ...
    void givenBeltId_whenGetBeltById_thenReturnBeltObject() throws Exception{
        ...
        Belt belt = beltRepository.save(
                new Belt(null,"JavaScript", "This is a JavaScript belt", Category.PROGRAMMING)
        );
        // when -  action or the behaviour that we are going test
    ...
    void givenUpdatedBelt_whenUpdateBelt_thenReturnUpdateBeltObject() throws Exception{
        ...
        Belt updatedBelt = new Belt(belt.getId(),"Python", "This is a Python belt", Category.PROGRAMMING);
        // when -  action or the behaviour that we are going test
        ...
        response.andExpect(status().isOk())
                ...
                .andExpect(jsonPath("$.category",
                        is(updatedBelt.getCategory().toString())));
    }
